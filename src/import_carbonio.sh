...
# 
# This script allows you migrate from Zimbra to Carbonio - it should work regardless of which version of Zimbra - it has been tested on Zimbra 8 and should be O/S independent
#
...
## Remember to make the script executable by running: chmod 755 /opt/scripts/import_carbonio.sh
...
sudo -u zextras bash -c 'export LC_ALL="en_US.UTF-8"'
...
if [ -z "$1" ]
then
      echo "Usage of this script : ./import_carbonio.sh <<source server IP>> <<directory>>(optional)"
      echo "Example Use : ]# ./import_carbonio.sh 111.222.333.444 /opt/zmbackup"
      echo "Example Use : ]# ./import_carbonio.sh 111.222.333.444"
      echo ""
...
echo ""
echo "TARGET Carbonio Server : localhost"
echo "TARGET Carbonio DIRECTORY : ${BACKUP_DIR}"
echo "--------------------------------------------------"
echo ""
...
if [ ! -d "$BACKUP_DIR" ]; then
  ...
  mkdir ${BACKUP_DIR}
  echo "Chown zextras:zextras ${BACKUP_DIR}..."
...
if [ ${RESPONSE_VAR} == "y" ]
  ...
  for i in `cat ${BACKUP_DIR}/domains.txt`; do sudo -u zextras /opt/zextras/bin/zmprov cd $i zimbraAuthMech zimbra ;echo $i ;done
  echo "Finished Importing Domains..."
  echo "Current Domain List:"
  sudo -u zextras /opt/zextras/bin/zmprov gad
  sleep 1
fi
...
if [ ${RESPONSE_VAR} == "y" ]
  ...
  tmpPass="CHang*ksl9"
  sudo -u zextras /opt/zextras/bin/zmprov ca $i ${tmpPass} cn "$givenName" displayName "$displayName" givenName "$givenName"
  sudo -u zextras /opt/zextras/bin/zmprov ma $i userPassword "$shadowpass"
  done
  echo "Finished Importing Domains..."
  echo "Current User List:"
  sudo -u zextras /opt/zextras/bin/zmprov -l gaa
  sleep 1
fi 
        ...
        echo "Signature import for $i"
          ...
          do
            ...
            if [[ ${LINE} == *"${SIGNATURENAME}"* ]]; then
              ...
              sudo -u zextras /opt/zextras/bin/zmprov csig $i "${zimbraSignatureName}"

              # Add HTML Signature 
              if [[ ${SIGNATURETYPE} == "HTML" ]]; then     
              echo "Execute Signature add /opt/zextras/bin/zmprov msig $i ${zimbraSignatureName} zimbraPrefMailSignatureHTML ${zimbraPrefMailSignatureHTML}"  
              sudo -u zextras /opt/zextras/bin/zmprov msig $i "${zimbraSignatureName}" zimbraPrefMailSignatureHTML "${zimbraPrefMailSignatureHTML}"
              fi
              
              if [[ ${SIGNATURETYPE} == "PLAIN" ]]; then  
              echo "Execute Signature add /opt/zextras/bin/zmprov msig $i ${zimbraSignatureName} zimbraPrefMailSignature ${zimbraPrefMailSignature}"  
              sudo -u zextras /opt/zextras/bin/zmprov msig $i "${zimbraSignatureName}" zimbraPrefMailSignature "${zimbraPrefMailSignature}" 
              fi
...
${LINE}"
    ...
    FILESIZE=$(stat -c%s "${BACKUP_DIR}/autoresponders/${i}_reply.txt")
        ...
        sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeReply "$AUTOREPLY1"
        echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeReply $AUTOREPLY1"  
        fi
    fi
    ...
    then
      FILESIZE=$(stat -c%s "${BACKUP_DIR}/autoresponders/$i.txt")
        ...
        if [ -n "$zimbraPrefOutOfOfficeReplyEnabled" ]
          then
        sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeReplyEnabled "$zimbraPrefOutOfOfficeReplyEnabled"
        echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeReplyEnabled $zimbraPrefOutOfOfficeReplyEnabled"
       fi
       ...
       zimbraFeatureOutOfOfficeReplyEnabled=$(grep zimbraFeatureOutOfOfficeReplyEnabled: ${BACKUP_DIR}/autoresponders/$i.txt | cut -d ":" -f2)
        ...
        if [ -n "$zimbraFeatureOutOfOfficeReplyEnabled" ]
          then
        sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraFeatureOutOfOfficeReplyEnabled "$zimbraFeatureOutOfOfficeReplyEnabled"
        echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraFeatureOutOfOfficeReplyEnabled $zimbraFeatureOutOfOfficeReplyEnabled"
       fi
        ...
        zimbraPrefOutOfOfficeFromDate=`echo $zimbraPrefOutOfOfficeFromDate | sed 's/^[[:space:]]*//; s/[[:space:]]*$//'`
          ...
          then
          sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeFromDate "$zimbraPrefOutOfOfficeFromDate"
          echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeFromDate $zimbraPrefOutOfOfficeFromDate"
          fi
        ...
        zimbraPrefOutOfOfficeCacheDuration=`echo $zimbraPrefOutOfOfficeCacheDuration | sed 's/^[[:space:]]*//; s/[[:space:]]*$//'`
          ...
          then
          sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeCacheDuration "$zimbraPrefOutOfOfficeCacheDuration"
          echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeCacheDuration $zimbraPrefOutOfOfficeCacheDuration"
          fi
        ...
        zimbraPrefOutOfOfficeUntilDate=`echo $zimbraPrefOutOfOfficeUntilDate | sed 's/^[[:space:]]*//; s/[[:space:]]*$//'`
          ...
          then
          sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeUntilDate "$zimbraPrefOutOfOfficeUntilDate"
          echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeUntilDate $zimbraPrefOutOfOfficeUntilDate"
          fi
        ...
        zimbraPrefOutOfOfficeStatusAlertOnLogin=`echo $zimbraPrefOutOfOfficeStatusAlertOnLogin | sed 's/^[[:space:]]*//; s/[[:space:]]*$//'`
          ...
          then
          sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeStatusAlertOnLogin "$zimbraPrefOutOfOfficeStatusAlertOnLogin"
          echo "run : sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefOutOfOfficeStatusAlertOnLogin $zimbraPrefOutOfOfficeStatusAlertOnLogin"
          fi
    ...
    then
      ...
      sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraMailSieveScript "`cat ${BACKUP_DIR}/filters/$i.txt`"
...
if [ ${RESPONSE_VAR} == "y" ]
      ...
      then
          ...
          do
             if [[ ${LINE} == *"zimbraMailForwardingAddress:"* ]]; then
              ...
              sudo -u zextras /opt/zextras/bin/zmprov ma $i -zimbraMailForwardingAddress "${zimbraMailForwardingAddress}"
              sudo -u zextras /opt/zextras/bin/zmprov ma $i +zimbraMailForwardingAddress "${zimbraMailForwardingAddress}"
  ...
  FILESIZE=$(stat -c%s "${BACKUP_DIR}/forwarders/${i}_userdefined.txt")
      ...
      sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraPrefMailForwardingAddress ${zimbraPrefMailForwardingAddress}
        ...
        then
            ...
            do
               if [[ ${LINE} == *"zimbraMailAlias:"* ]]; then
                ...
                sudo -u zextras /opt/zextras/bin/zmprov aaa $i ${zimbraMailAlias}
          ...
          sudo -u zextras /opt/zextras/bin/zmprov CreateDistributionList $i 
              ...
              do
                 ...
                 if [[ ${LINE} == *"zimbraMailForwardingAddress:"* ]]; then
                  ...
                  sudo -u zextras /opt/zextras/bin/zmprov AddDistributionListMember $i "${zimbraMailForwardingAddress}"
                 fi
              ...
              for pref in "${IMPORT_PREF[@]}"
               ...
               if [[ ${LINE} == *"$REGEXPREF"* ]]; then
                ...
                echo "Modify Pref for $i ${pref} $PREFVALUE"
                  ...
                  sudo -u zextras /opt/zextras/bin/zmprov ma $i +$pref $PREFVALUE
                  echo "Execute :  sudo -u zextras /opt/zextras/bin/zmprov ma $i +$pref $PREFVALUE"  
        ...
        while read -r LINE
          do
             if [[ ${LINE} == *"zimbraInterceptAddress:"* ]]; then
              ...
              sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraInterceptAddress $intercept
            ...
            if [[ ${LINE} == *"zimbraSharedItem:"* ]]; then
                ...
                #  do
                  ...
                  if [[ -n "$shareRights" && -n "$shareFolder" && -n "$shareGrantee" && -n "$shareGranteeType" ]]; then 
                    ...
                    echo "Execute"
                    echo "sudo -u zextras /opt/zextras/bin/zmmailbox -z -m $i mfg $shareFolder account $shareGrantee $shareRights"
                    sudo -u zextras /opt/zextras/bin/zmmailbox -z -m $i mfg "${shareFolder}" account "$shareGrantee" "$shareRights"
                    echo "Executed"
                    fi
                  # Reset all variables
                  shareType=""
        ...
        while read -r LINE
          do
             if [[ ${LINE} == *"zimbraAccountStatus:"* ]]; then
              ...
              sudo -u zextras /opt/zextras/bin/zmprov ma $i zimbraAccountStatus $status
